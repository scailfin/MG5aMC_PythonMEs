...
class UFOModelConverterPython(export_cpp.UFOModelConverterCPP):
    ...
    def write_aloha_routines(self):
        ...
        wavefunction_routines = open(
            pjoin(MG5DIR, "aloha", "template_files", "wavefunctions.py"), "r"
        ).read()
...
"""
        ...
        shutil.copytree(
            pjoin(plugin_path, "templates", "phase_space_generator"),
    ...
    def finalize(self, matrix_elements, history, options, flaglist):
        ...
        open(check_sa_path, "w").write(
            open(pjoin(plugin_path, "templates", self.check_sa_template), "r")
            .read()
            .format(**replace_dict)
        )
